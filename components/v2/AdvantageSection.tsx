...
/* ─────────── Theme helpers (static — computed once) ─────────── */
const THEME_MAP = {
  blue: {
...
/* ─────────── Individual Card ─────────── 
   Uses a SINGLE useScroll + pure useTransform chain.
   Zero state, zero callbacks, zero re-renders during scroll. */
function AdvantageCard({ item, index }: { item: AdvantageItem; index: number }) {
  ...
  // Cards visible immediately — only subtle fade/shift as they scroll past
  ...
  return (
    ...
    >
      <div className={cn(
        "relative rounded-[32px] p-8 md:p-10 overflow-hidden",
        "bg-white border-2 shadow-[0_32px_64px_-16px_rgba(0,0,0,0.06)]",
        theme.border
      )}>
        <div className="relative z-10 flex flex-col gap-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="group relative p-6 rounded-3xl bg-slate-100/50 border border-slate-200 shadow-sm transition-colors hover:bg-slate-200/50">
            ...
            <div className={cn(
              "group relative p-6 rounded-3xl shadow-lg border",
              `bg-gradient-to-br ${theme.gradient}`,
              "hover:shadow-xl hover:-translate-y-1 transition-[box-shadow,transform] duration-300"
            )}>
              <div className="absolute inset-0 bg-white/5 opacity-0 group-hover:opacity-100 transition-opacity rounded-3xl" />
              <div className="flex items-center gap-2 mb-4 relative z-10">
                <div className="p-1 px-2.5 rounded-md bg-white/20 text-u-xs font-bold text-white uppercase tracking-u-widest border border-white/20">
                  MediKloud
                </div>
...
/* ─────────── Step Indicator ───────────
   Uses IntersectionObserver (native browser API) instead of framer-motion
   for the active index — ultra lightweight, fires only on threshold cross. */
                ...
                >
                  <div className="px-5 pb-5 pt-0 space-y-4 border-t border-slate-50">
                    ...
                    <div className="grid grid-cols-1 gap-3">
                      ...
                      <div className={cn(
                        "p-4 rounded-2xl text-white shadow-md relative overflow-hidden bg-gradient-to-br",
                        theme.gradient
                      )}>
...
/* ─────────── Main Section ───────────
   Uses native IntersectionObserver for active-index tracking.
   This fires ONLY when a card crosses the viewport center — not on every frame.
   Cards animate via pure framer-motion transforms (no React state during scroll). */
export function AdvantageSection() {
  const containerRef = useRef<HTMLElement>(null);
  const cardRefs = useRef<(HTMLDivElement | null)[]>([]);
  const [activeCardIndex, setActiveCardIndex] = useState(0);

  // Native IntersectionObserver — fires only on threshold crosses, not every frame
  useEffect(() => {
    ...
    cardRefs.current.forEach((el, index) => {
      ...
      const observer = new IntersectionObserver(
        ...
        {
          // Trigger when card hits the center 30% of the viewport
  ...
  return (
    ...
    >
      {/* Background Decor — lighter blurs for performance */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-0 right-[-5%] w-[400px] h-[400px] bg-blue-50/40 rounded-full blur-[60px]" />
        <div className="absolute bottom-[-5%] left-[-5%] w-[350px] h-[350px] bg-indigo-50/40 rounded-full blur-[60px]" />
      </div>
