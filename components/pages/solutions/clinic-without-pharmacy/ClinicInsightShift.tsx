      ...
      <div className="container-page relative z-10">
        ...
        >
          <div className="eyebrow-wrap justify-center">
            <span className="eyebrow-line-r" style={{ background: 'rgba(99,102,241,0.3)' }} />
            <span className="text-xs font-semibold uppercase tracking-[0.15em] text-indigo-400 flex items-center gap-2">
              <Sparkles className="w-3.5 h-3.5" /> The MediKloud Virtual Pharmacy
            </span>
            <span className="eyebrow-line-l" style={{ background: 'rgba(99,102,241,0.3)' }} />
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8">
          {[
            ...
            <div
              key={i}
              className="card-dark--flush backdrop-blur-xl rounded-3xl p-10 hover:bg-white/10 hover:border-indigo-500/50 transition-all duration-500 group"
            >
              <div className="w-14 h-14 rounded-2xl bg-indigo-500/10 border border-indigo-500/20 flex items-center justify-center mb-8 group-hover:bg-indigo-500/20 group-hover:border-indigo-500/30 transition-all duration-500">
                <item.icon className="w-7 h-7 text-indigo-400" strokeWidth={1.5} />
              </div>
              <h3 className="premium-h3 text-white mb-3">{item.title}</h3>
              <p className="premium-p text-slate-400">{item.desc}</p>
            </div>
          ))}
        </div>
      </div>
