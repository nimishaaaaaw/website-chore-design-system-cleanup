...
export const WhatsAppMockup = () => {
  ...
  return (
    <div className="w-full max-w-[280px] bg-[#0b141a] border-[8px] border-slate-800/80 rounded-[2.5rem] shadow-2xl relative z-10 overflow-hidden flex flex-col h-[500px]">
      ...
      {/* WhatsApp Header */}
      <div className="bg-[#202c33] pt-7 pb-2.5 px-3 flex items-center gap-3 relative z-20 shadow-md">
        ...
        <div className="flex-1">
          <div className="text-[#e9edef] text-[13px] font-bold flex items-center gap-1.5 leading-tight">
      ...
      <div key={loopKey} ref={chatRef} className="flex-1 bg-[#0b141a] p-3 flex flex-col gap-2.5 overflow-y-auto hide-scrollbar relative z-10 pb-6">
        {/* Abstract Chat Pattern Overlay */}
        <div className="absolute inset-0 opacity-5 pointer-events-none" style={{ backgroundImage: "radial-gradient(#ffffff 1px, transparent 1px)", backgroundSize: "12px 12px" }}></div>
        
        {/* Date Pill */}
        <div className="flex justify-center mt-1 mb-2">
          <span className="bg-[#182229] text-[#8696a0] text-[10px] px-3 py-1 rounded-lg relative z-10">Today</span>
        </div>
        ...
        {step >= 1 && (
          <motion.div 
            ...
            className="bg-[#202c33] rounded-2xl rounded-tl-sm shadow-sm self-start w-[88%] relative z-10 flex flex-col overflow-hidden border border-[#313d45]/50"
          >
            <div className="p-3 pb-1.5">
              <div className="text-[12.5px] text-[#e9edef] leading-snug">
                Hi! Your 30-day course of <span className="font-bold">Telmisartan 40mg</span> is ending soon. 
                <br/><br/>
            ...
            {/* Interactive Buttons */}
            <div className="border-t border-[#313d45] flex flex-col mt-1">
              <div className="py-2.5 text-center text-[13px] font-medium text-[#53bdeb] border-b border-[#313d45] relative overflow-hidden cursor-default">
                {/* Simulated tap flash effect just before moving to step 2 */}
                {step === 1 && (
                   <motion.div 
                     initial={{ opacity: 0 }} animate={{ opacity: [0, 0.15, 0] }} transition={{ delay: 1.6, duration: 0.2 }}
        ...
        {step >= 3 && (
          <motion.div 
            initial={{ opacity: 0, x: -10, scale: 0.95, transformOrigin: 'top left' }} animate={{ opacity: 1, x: 0, scale: 1 }} 
            className="bg-[#202c33] rounded-2xl rounded-tl-sm p-2.5 shadow-sm self-start w-[92%] relative z-10 mt-2 border border-[#313d45]/50"
          >
            <div className="text-[12.5px] text-[#e9edef] leading-snug mb-3">
              ✅ <span className="font-bold">Order Confirmed!</span><br/>
              Your medicines will arrive by 5 PM.
            </div>
            
            {/* Invoice Card embedded in message */}
            <div className="bg-[#0b141a] rounded-xl border border-[#313d45] overflow-hidden">
              <div className="p-3 border-b border-[#313d45] bg-white/[0.02]">
                <div className="text-[10px] text-[#8696a0] uppercase tracking-wider mb-1.5 font-bold">Invoice Details</div>
                <div className="flex justify-between items-center">
                    <span className="text-[13px] text-[#e9edef] font-medium">Total Amount</span>
                    <span className="text-[14px] text-emerald-400 font-bold">₹850</span>
                </div>
              </div>
              <div className="flex divide-x divide-[#313d45] relative">
                {/* Simulated tap on "Pay Now" just before step 4 */}
                {step === 3 && (
                   <motion.div 
                     initial={{ opacity: 0 }} animate={{ opacity: [0, 0.15, 0] }} transition={{ delay: 2.6, duration: 0.2 }}
                     className="absolute inset-y-0 right-0 w-1/2 bg-white"
                   />
                )}
                <div className="flex-1 py-2.5 text-center text-[12.5px] font-medium text-[#53bdeb] cursor-default">View Bill</div>
                <div className="flex-1 py-2.5 text-center text-[12.5px] font-medium text-[#53bdeb] cursor-default">Pay Now</div>
              </div>
            </div>
        ...
        {step >= 4 && (
          <motion.div 
             initial={{ opacity: 0, x: -10, scale: 0.95, transformOrigin: 'top left' }} animate={{ opacity: 1, x: 0, scale: 1 }}
             className="bg-[#202c33] rounded-2xl rounded-tl-sm p-2.5 shadow-sm self-start w-[85%] relative z-10 mt-1 border border-[#313d45]/50"
          >
        ...
        {step >= 5 && (
          <motion.div 
            ...
            className="bg-[#202c33] rounded-2xl rounded-tl-sm p-1 shadow-sm self-start w-[92%] relative z-10 mt-1 border border-[#313d45]/50"
          >
