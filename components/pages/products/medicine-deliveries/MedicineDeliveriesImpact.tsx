      ...
      <div className="container-page relative z-10">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6">
          {impactBenefits.map((benefit, index) => (
            <div
              key={benefit.id}
              className="card-dark rounded-3xl p-7 lg:p-9 hover:bg-white/10 hover:border-indigo-500/50 transition-all duration-500 relative group overflow-hidden"
            >
              {/* Hover glow blob */}
              <div className="absolute top-0 right-0 w-32 h-32 bg-indigo-500/10 rounded-full blur-3xl -mr-16 -mt-16 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none" />

              <div className="relative mb-10 flex justify-between items-center">
                <div className="w-12 h-12 bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 rounded-xl flex items-center justify-center shadow-card group-hover:scale-110 transition-transform duration-500">
                  {benefit.icon}
                </div>
                <div className="text-xs font-bold text-slate-500 group-hover:text-indigo-300 bg-white/5 px-2.5 py-1 rounded-lg transition-colors border border-white/5">
                  {benefit.id}
                </div>
              </div>

              <h3 className="text-h2 font-bold text-white mb-3 tracking-tight group-hover:text-indigo-400 transition-colors leading-tight">
                {benefit.title}
              </h3>
