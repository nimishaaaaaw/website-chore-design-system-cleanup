...
export function HeaderVariant({ variant, title }: HeaderProps) {
  ...
  return (
    <div className="w-full mb-20 border rounded-xl overflow-hidden shadow-sm">
      {/* Preview label bar */}
      <div className="bg-slate-100 px-4 py-2 border-b text-xs font-mono text-slate-400 flex justify-between items-center">
        <span>Option: {title} ({variant} palette)</span>
        <span className="bg-white px-2 py-0.5 rounded border border-slate-200 text-slate-400">
          Preview Only
        </span>
      </div>
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <nav className="hidden md:flex flex-1 justify-center items-center gap-8">
              {['Products', 'Solutions', 'About Us', 'Blogs'].map((item) => (
                ...
                >
                  {item}
                  <span className={`absolute left-0 -bottom-1 h-[2px] w-full ${s.underline} scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-left`} />
                </Link>
              ))}
            </nav>
            ...
            <div className="flex-shrink-0 w-48 flex justify-end">
              <button className={`${s.button} px-5 py-2 text-sm font-bold rounded-xl transition-all shadow-sm`}>
                Talk to us
              </button>
            </div>
