        ...
        <div className="space-y-10">
            {blocks.map((block, index) => {
                ...
                if (trimmed.startsWith('## ')) {
                    ...
                    const id = slugify(text)
                    return (
                        <h2 key={index} id={id} className="scroll-mt-32 text-h2 md:text-h1 font-bold pt-12 pb-4 text-slate-900 flex items-center group">
                ...
                if (trimmed.startsWith('### ')) {
                    ...
                    const id = slugify(text)
                    return (
                        <h3 key={index} id={id} className="scroll-mt-32 text-h3 font-bold pt-8 pb-2">
                            {text}
                        </h3>
                    )
                }
                        ...
                        <ul key={index} className="space-y-5 my-8">
                            {items.map((item, i) => (
                                <li key={i} className="flex items-start gap-4">
                                    ...
                                    <span className="text-body-lg text-slate-600 leading-relaxed">
...
function parseInlineStyles(text: string) {
    ...
    return linkParts.map((part, i) => {
        ...
        if (linkMatch) {
            ...
            return (
                <Link 
                    key={i} 
                    href={href} 
                    className="text-brand-600 hover:text-brand-700 decoration-brand-100 hover:decoration-brand-500"
        ...
        return codeParts.map((subPart, j) => {
            ...
            if (codeMatch) {
                return (
                    <code key={`${i}-${j}`} className="inline-code">
                        {codeMatch[1]}
                    </code>
                )
            }
